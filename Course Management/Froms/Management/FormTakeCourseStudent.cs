...
{
    public partial class FormTakeCourseStudent : Form
    {
        DatabaseContainer db = new DatabaseContainer();
        private FromManagement form;
        private Student student;

        List<Course> tempCourses = new List<Course>();

        public FormTakeCourseStudent(FromManagement form, Student student)
        {
            InitializeComponent();
            this.form = form;
        ...
        private void btn_ok_Click(object sender, EventArgs e)
        {
            ...
            db.Students.Find(student.Id).Courses.Clear();
            foreach (Course c in tempCourses)
            {
                student.Courses.Add(c);
                db.Students.Find(student.Id).Courses.Add(c);
            }

            db.SaveChanges();
            form.loadStudents();
            MessageBox.Show("انتخاب واحد انجام شد");
            Close();
        }
    }
}
